...
class FFmpegWrapper:
    ...
    def estimate_size(
        self, duration: float, video_bitrate: str, audio_bitrate: str = "128k"
    ) -> int:
    ...
    def build_command(
        self, input_file: Path, output_file: Path, preset: Optional[str] = None, **kwargs
    ) -> List[str]:
        ...
        if "ffmpeg_preset" in kwargs:
            cmd.extend(["-preset", kwargs["ffmpeg_preset"]])
...
def interactive_mode(input_file: Path):
    ...
    if choice == 1:  # Compress
        ...
        table.add_row("Optimized", "???", "Smart CRF optimization (Recommended)")

        console.print(table)

        quality = Prompt.ask(
            "\n[cyan]Choose quality[/cyan]",
            choices=["high", "balanced", "maximum", "optimized"],
            default="optimized",
        )
        ...
        if ffmpeg.run_with_progress(cmd, info.duration):
            ...
            console.print(
                f"   Saved: {format_size(info.size - new_size)} ({100 - (new_size / info.size) * 100:.1f}% reduction)"
            )
        ...
        cmd = ffmpeg.build_command(
            input_file, output_file, start_time=start_time, end_time=end_time
        )
        ...
        format_choice = Prompt.ask(
            "\n[cyan]Output format[/cyan]", choices=["mp3", "aac", "wav", "flac"], default="mp3"
        )
...
def trim(input_file, start_time, end_time, output):
    ...
    cmd = ffmpeg.build_command(input_path, output_path, start_time=start_time, end_time=end_time)
...
def info(input_file):
    ...
    if info.resolution_name in ["4K", "2K"]:
        recommendations.append(
            "• File is high-res - [cyan]mobile[/cyan] preset recommended for sharing"
        )

    if info.video_codec not in ["h264", "libx264", "h265", "libx265"]:
        recommendations.append(
            "• Non-standard codec - consider [cyan]convert[/cyan] for better compatibility"
        )
...
def presets():
    ...
    for key, preset in PRESETS.items():
        table.add_row(key, preset["desc"], use_cases.get(key, ""))
